...
#   USB_MAXCONFIG
  ...
  class DevHandle
    ...
    def usb_set_altinterface(i, c); @dev.set_interface_alt_setting(i, c); end
    ...
    def usb_control_write(requesttype, request, value, index, bytes, timeout)
      @dev.control_transfer(:bmRequestType=>requesttype, :bRequest=>request, :wValue=>value,
          :wIndex=>index, :dataOut=>bytes, :timeout=>timeout)
    end
    def usb_control_read(requesttype, request, value, index, bytes, timeout)
      @dev.control_transfer(:bmRequestType=>requesttype, :bRequest=>request, :wValue=>value,
...
#   rb_define_method(rb_cUSB_DevHandle, "usb_get_descriptor_by_endpoint", rusb_get_descriptor_by_endpoint, 4);
    ...
    def set_altinterface(interface, alternate)
      ...
      self.usb_set_altinterface(interface, alternate)
    end
